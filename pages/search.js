                ...
                <div>
                    ...
                    <div className="mb-3">
                        ...
                        <select className="w-full" value={price} onChange={priceHandler}>
                            ...
                            {prices &&
                            prices.map((price) => (
                                <option key={price.value} value={price.value}>{price.name}</option>
                            ))}
                        </select>
                    </div>
                    <div className="mb-3">
                        ...
                        <select className="w-full" value={rating} onChange={ratingHandler}>
                            ...
                            {ratings &&
                            ratings.map((rating) => (
                                <option key={rating} value={rating}>{rating} star{rating > 1 && 's'} & up</option>
                            ))}
                        </select>
                    </div>
                </div>
                <div className="md:col-span-3">
                    <div className="mb-2 flex items-center justify-between border-b-2 scroll-pb-2">
                    ...
                    <ul className="flex">
                        {products.length > 0 && [...Array(pages).keys()].map((pageNumber) => (
                            <li key={pageNumber}>
                                <button className={`default-button m-2 ${page == pageNumber + 1 ? 'font-bold' : ''}`} onClick={() => pageHandler(pageHandler + 1)}>
                                    {pageNumber + 1}
                                </button>
                            </li>
                        ))}
...
export async function getServerSideProps({ query }) {
    ...
    await db.disconnect();
    //convert document to document object using convertDocToObj function
